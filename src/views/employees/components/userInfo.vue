      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <!-- 放置上传图片 -->
            <ImageUpload ref="userInfoStaffPhoto"></ImageUpload>
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <ImageUpload ref="formDataStaffPhoto"></ImageUpload>
        </el-form-item>
...
export default {
  ...
  async created() {
    ...
    if (this.userInfo.staffPhoto) {
      this.$refs.userInfoStaffPhoto.fileList = [{ url: this.userInfo.staffPhoto }]
    }
    if (this.formData.staffPhoto) {
      this.$refs.formDataStaffPhoto.fileList = [{ url: this.formData.staffPhoto }]
    }
  },
  methods: {
    async saveUser() {
      const { isProgress, fileList } = this.$refs.userInfoStaffPhoto
    ...
    async savePersonal() {
      const { isProgress, fileList } = this.$refs.formDataStaffPhoto
      if (fileList.length > 0) {
